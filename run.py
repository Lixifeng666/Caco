...
def main(args):
    ...
    print("data['input_dim']",data['input_dim'],"len(data['features'])",len(data['features']))
    model = init_model(args, data['input_dim'],data['add_input_dim'], len(data['features']))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lamda1", type=float, default=0.5,
                        help="Weight for attribute")
    parser.add_argument("--lamda2", type=float, default=0.5,
                        help="Weight for attribute")
    parser.add_argument("--lamda3", type=float, default=0.5,
                        help="Weight for attribute")
    ...
    for dataset_name in list(DATASETS_NAME.keys()):
        ...
        print("Model\tDataset\tlayers\tembedding\tlamda2\tlamda3\tNormal_Label\tAUC_mean\tAUC_std\tAP_mean\tAP_std\tMax_Epoch",
              file=file2print_detail)

        print("Model\tDataset\tlayers\tembedding\tlamda2\tlamda3\tTest\tAUC_mean\tAUC_std\tAP_mean\tAP_std\tMax_Epoch")
        print("Model\tDataset\tlayers\tembedding\tlamda2\tlamda3\tTest\tAUC_mean\tAUC_std\tAP_mean\tAP_std\tMax_Epoch", file=file2print)
        file2print.flush()
        ...
        for m in lam2:
            ...
            for n in lam3:
                ...
                for i in lay:
                    ...
                    for j in emb:
                        ...
                        for normal_idx in range(DATASETS_NAME[dataset_name]):
                            ...
                            print("{}\t{}\t{:.1f}\t{:.1f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{}".format(
                                args.module, dataset_name, args.n_layers, args.n_hidden,args.lamda2,args.lamda3,normal_idx, AUCs_seed_mean,
                ...
                print("去掉不变项{}\t{}\t{:.1f}\t{:.1f}\t{:.4f}\t{:.4f}\tTest\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{}".format(
                    args.module, dataset_name, args.n_layers, args.n_hidden,args.lamda2,args.lamda3,
